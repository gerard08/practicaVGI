...
BEGIN_MESSAGE_MAP(CEntornVGIView, CView)
	...
	ON_UPDATE_COMMAND_UI(ID_OBJECTE_ARC, &CEntornVGIView::OnUpdateObjecteArc)
		...
		ON_COMMAND(ID_TEXTURA_MAPA, &CEntornVGIView::OnTexturaMapa)
		ON_UPDATE_COMMAND_UI(ID_TEXTURA_MAPA, &CEntornVGIView::OnUpdateTexturaMapa)
		ON_COMMAND(ID_TEXTURA_GALLINAS, &CEntornVGIView::OnTexturaGallinas)
		ON_UPDATE_COMMAND_UI(ID_TEXTURA_GALLINAS, &CEntornVGIView::OnUpdateTexturaGallinas)
		END_MESSAGE_MAP()
...
// PLANTA (Inferior Esquerra)
		...
		ProjectionMatrix = Projeccio_Orto(0, 0, (w/2.0)-1, (h/2.0)-1);
		...
		//ProjectionMatrix = glm::scale(ProjectionMatrix, vec3(100,100,100));
		///ViewMatrix = glm::scale(ViewMatrix, vec3(0,0,0));
		//ViewMatrix = glm::scale(ViewMatrix, vec3(mida, mida, mida));
...
// ISOMÈTRICA (Inferior Dreta)
		// Definició de Viewport, Projecció i Càmara
		ProjectionMatrix = Projeccio_Orto((w/2)+1, 0, (w/2)-1, (h/2)-1);
...
// ALÇAT (Superior Esquerra)
		// Definició de Viewport, Projecció i Càmara
		ProjectionMatrix = Projeccio_Orto(0, (h/2)+1, (w/2)-1, (h/2)-1);
...
// PERFIL (Superior Dreta)
		// Definició de Viewport, Projecció i Càmara
		ProjectionMatrix = Projeccio_Orto((w/2)+1, (h/2)+1, (w/2)-1, (h/2)-1);
...
//	Dibuix geometria de l'escena amb comandes GL.
	dibuixa_EscenaGL(shader_programID, eixos, eixos_Id, grid, hgrid, objecte, col_obj, true, sw_material, textura, texturesID, textura_map,
		npts_T, PC_t, pas_CS, sw_Punts_Control, ViewMatrix, GTMatrix);
...
void CEntornVGIView::OnTexturaMapa()
{
	t_textura = MAPA;
	wglMakeCurrent(m_pDC->GetSafeHdc(), m_hRC);
...
void CEntornVGIView::OnUpdateTexturaMapa(CCmdUI* pCmdUI)
{
	// TODO: Agregue aquí su código de controlador de IU para actualización de comandos
	if (t_textura == MAPA) pCmdUI->SetCheck(1);
	else pCmdUI->SetCheck(0);
}


void CEntornVGIView::OnTexturaGallinas()
{
	t_textura = GALLINAS;
	wglMakeCurrent(m_pDC->GetSafeHdc(), m_hRC);


	texturesID[0] = loadIMA_SOIL("./textures/gallinas.jpg");
	//	Pas de textura al shader
	if (shader_menu != CAP_SHADER) glUniform1i(glGetUniformLocation(shader_programID, "texture0"), GLint(0));
...
void CEntornVGIView::OnUpdateTexturaGallinas(CCmdUI* pCmdUI)
{
	// TODO: Agregue aquí su código de controlador de IU para actualización de comandos
	if (t_textura == GALLINAS) pCmdUI->SetCheck(1);
	else pCmdUI->SetCheck(0);
}
