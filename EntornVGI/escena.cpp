...
// Tetera
	case TETERA:
		//glPushMatrix();
		  //glScaled(5.0,5.0,5.0);
		  ModelMatrix = MatriuTG;
		  //ModelMatrix = glm::rotate(ModelMatrix, glm::radians(0.f), vec3(0, 0, 1));
		  ModelMatrix = glm::rotate(ModelMatrix, glm::radians(90.f), vec3(1, 0, 0));
		  ModelMatrix = glm::translate(ModelMatrix, vec3(0, -2, 0));
		  // Pas ModelView Matrix a shader
		  glUniformMatrix4fv(glGetUniformLocation(sh_programID, "modelMatrix"), 1, GL_FALSE, &ModelMatrix[0][0]);
		  // Pas NormalMatrix a shader
		  NormalMatrix = transpose(inverse(MatriuVista * ModelMatrix));
...
void pop(GLuint sh_programID, glm::mat4 MatriuVista, glm::mat4 MatriuTG, bool sw_mat[4])
{
	...
	CColor col_object;
	col_object.r = 0.5;		col_object.g = 0.5;		col_object.b = 0.5;		col_object.a = 1.0;//gris
	SeleccionaColor(sh_programID, col_object, sw_mat);
